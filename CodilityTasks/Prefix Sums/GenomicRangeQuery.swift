...
public func solution(inout S : String, inout _ P : [Int], inout _ Q : [Int]) -> [Int] {
    ...
    for (index, letter) in characters.enumerate() {
        for factor in 0...3 {
            prefixSums[index + 1][factor] += prefixSums[index][factor]
        }
        let value = impactFactors[letter]! - 1
        prefixSums[index+1][value] += 1
    }
    
    var results = Array(count: P.count, repeatedValue: 0)
    for index in 0...P.count - 1 {
        let start = P[index]
        let end = Q[index] + 1
        var result = 4
        for factor in 0...3 {
            let startFactor = prefixSums[start][factor]
            let endFactor = prefixSums[end][factor]
            if endFactor - startFactor > 0 {
                result = factor + 1
                break
            }
        }
